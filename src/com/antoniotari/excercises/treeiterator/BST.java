...
public class BST<K extends Comparable<K>, V> {
	...
	private class Node {
		...
		private int totNodes;             // number of nodes in subtree

		public Node(K key, V val, int totNodes) {
			this.key = key;
			this.val = val;
	...
	private Node put(Node x, K key, V val) {
		...
		x.totNodes = 1 + size(x.left) + size(x.right);
		return x;
	}
	...
	private Node deleteMin(Node x) {
		...
		x.totNodes = size(x.left) + size(x.right) + 1;
		return x;
	}
	 ...
	 *
	 * @return all keys in the symbol table between <tt>lo</tt> 
	...
	private boolean check() {
		if (!isBST())            L.log("Not in symmetric order");
		if (!isSizeConsistent()) L.log("Subtree counts not consistent");
		if (!isRankConsistent()) L.log("Ranks not consistent");
	...
	private boolean isSizeConsistent(Node x) {
		if (x == null) return true;
		if (x.totNodes != size(x.left) + size(x.right) + 1) return false;
